...
class singer{
    ...
    function select1(){
        $rid=$_REQUEST['rid'];
        $result=$this->db->query("select * from singer where rid=$rid")->fetch_assoc();
        $data=$this->db->query("select sort.* , singer.*, song.* from sort,singer,song where singer.rid=$rid and song.rid=$rid")->fetch_all(1);
